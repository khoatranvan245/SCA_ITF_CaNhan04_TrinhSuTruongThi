...
function buildCandidateAvatarFileName(candidateId) {
...
function extractResumePath(fileUrl) {
...
async function toSignedResumeUrl(fileUrl) {
...
export const getCandidateProfile = async (req, res) => {
    try {
        ...
        const resumes = await Promise.all(candidate.resumes.map(async (resume) => ({
            resume_id: resume.resume_id,
            name: resume.name,
            file_url: await toSignedResumeUrl(resume.file_url),
            uploaded_at: resume.uploaded_at,
        })));
        const avatarUrl = await formatAvatarUrl(candidate.avatar_url, user.updated_at);
        res.status(200).json({
            candidate: {
...
export const updateCandidateProfile = async (req, res) => {
    try {
        ...
        const skillsInput = Array.isArray(rawSkills)
            ? rawSkills
        ...
        res.status(200).json({
            ...
            candidate: {
                ...
                avatar_url: await formatAvatarUrl(updatedCandidate.avatar_url, user.updated_at),
...
export const uploadCandidateAvatar = async (req, res) => {
    try {
        await prisma.$connect();
        const userId = parseUserId(req.params.userId);
        if (!userId) {
        ...
        const avatarBuffer = await sharp(file.buffer)
            ...
            .toBuffer();
        const objectPath = `candidates/${buildCandidateAvatarFileName(candidate.candidate_id)}`;
        const { error: uploadError } = await supabaseAdmin.storage
            .from(AVATAR_BUCKET_NAME)
        ...
        const avatarUrl = await formatAvatarUrl(updatedCandidate.avatar_url, user.updated_at);
        res.status(200).json({
...
export const deleteCandidateResume = async (req, res) => {
    try {
        ...
        const targetResume = candidate.resumes.find((resume) => resume.resume_id === resumeId);
        if (!targetResume) {
